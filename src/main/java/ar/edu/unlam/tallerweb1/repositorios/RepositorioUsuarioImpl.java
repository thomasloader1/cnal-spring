...
public class RepositorioUsuarioImpl implements RepositorioUsuario {
	...
	@Override
	public Boolean unirseAPartido(Long idUsuario, Long idPartido) {
		final Session session = this.sessionFactory.getCurrentSession();
		Usuario usuario = session.get(Usuario.class, idUsuario);
		Partido partido = session.get(Partido.class, idPartido);
		Boolean estado = usuario.agregarPartido(partido);
	...
	public List<Usuario> listarJugadoresPorPartido(Long idPartido) {
		...
		if(usuarioPartidos.size() > 0) {
			for (UsuarioPartido usuarioPartido : usuarioPartidos) {
				...
				Usuario usuario = this.buscarUsuarioPorId(usuarioPartido.getIdUsuario());
